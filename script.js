...
let myarr = []
let yarr = []
for(let ch in json.data){
  myarr.push(Number(json.data[ch][1]))
  yarr.push(json.data[ch][0][0]+json.data[ch][0][1]+json.data[ch][0][2]+json.data[ch][0][3])
}
const w = 900;
    const h = 460;
...
const svg = d3.select("#chart")
    ...
    .enter()
    .append("rect")
    .attr("x",(d,i)=>padding+i*(830/myarr.length))
    .attr("y",(d,i)=>h-padding-yScale(1*d))
    .attr('width',2.909090909090909)
    .attr("height",(d)=>yScale(1*d))
    .attr("fill", "navy")
    .attr('class','bar')
    .attr('date-gdp',(d)=>d)
    .append('title')
    ...
    let extent = d3.extent(yarr)
    const xScale2 = d3.scaleLinear()
                     .domain(extent)
                     .range([padding, w - padding]);
const xAxis = d3.axisBottom(xScale2);
let extent3 = d3.extent(myarr)
